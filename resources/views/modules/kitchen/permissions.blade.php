        ...
        <div class="card">
            <div class="card-header header-elements-inline">
                ...
                <div class="header-elements">
                    <button class="btn bg-success addPermission">
                        <i class="icon-add"></i>
                        Tambah
                    </button>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-sm dtpermissions" style="width: 100%">
                    <thead>
...
@section('scripts')
    <script>
        $(function () {
            let dtpermissions = $('.dtpermissions').DataTable({
                "scrollX": true,
                "searching": false,
                "ordering": false,
                scrollY: 350,
                ...
                "rowCallback": function (row, data) {
                    $('td:eq(4)', row).html('<div class="btn-group">'+
                        '<button type="button" class="btn btn-sm btn-success editPermission"'+
                        ' data-id="'+data.id+'" data-name="'+data.name+'"'+
                        ' data-display_name="'+data.display_name+'"'+
                        ' data-description="'+data.description+'">'+
                        '<i class="fa fa-edit"></i> Sunting'+
                        '</button>'+
                        '<button type="button" class="btn btn-sm btn-danger deletePermission"'+
                        ' data-id="'+data.id+'">'+
                ...
                "drawCallback": function(){
                    ...
                    $('.deletePermission').on('click', function(e){
                        ...
                        swal({
                            ...
                            text: "Permission akan dihapus. Silakan ganti user dengan role tersebut!",
                            type: 'warning',
                            ...
                            if(result.value) {
                                $.ajax({
                                    url: '{!! route('kitchen.permissions.delete') !!}',
                                    type: "DELETE",
                                    data: {
                                        id: id
                                    },
            ...
            $('.saveBtn').on('click', function (e) {
                ...
                if ($.trim(name) === '') {
                    ...
                    if ($.isNumeric(id)) {
                        url = '{{ route("kitchen.permissions.update") }}';
                        method = 'PATCH';
                    } else {
                        url = '{{ route("kitchen.permissions.create") }}'
                        method = 'POST'
                    }
                    $.ajax({
                        url: url,
